...
#	Written and tested under OSX10.6.4
#	Current R version 2.11.1
#	
#	Alan Mackay
#
...
#	Libraries required
#
## Edited by Charlotte Ng
## Largely untested
## Change log (20130613)
## - readPheno: When correlation is used as the distance metric, the dist object is as.dist(1-cor()) not dist(1-cor())
## - dietSAM2Class: Bug fixed for when there is one DE gene
## Change log (20131105)
## cghheatmap: when is.null(cluster), order is not retained (not reversed)
...
#	commands <- c(cgh$commands,deparse(match.call()))
	...
	MB <- calculateMB(cgh)
	if(sum(is.na(exprs(cgh))) > 0) exprs(cgh) <- imputeCGH(cgh, span=2)
...
latticePlotFishers <- function(cgh, device="PDF", project, plot.adjusted.p=T, LOOCV=F, pval.thresh=0.05)
{
require(Biobase)
library(lattice)
library(grid)
library(marray)
	...
	scales=list(x=list(at=mids,labels=chr.nms, cex=0.5)),
	panel = function(...)
...
#	commands <- c(exp.cgh$commands,deparse(match.call()))
	...
	for (i in 1:nrow(exp.cgh))
		...
		{
            if (sum(is.na(exprs(exp.cgh[i,]))) == ncol(exp.cgh) || sum(is.na(assayData(exp.cgh)$smo[i,])) == ncol(exp.cgh)) {
                next
            }
...
#	commands <- c(exp.cgh$commands,deparse(match.call()))
	...
	for (i in 1:nrow(exp.cgh))
		{
        if (sum(is.na(exprs(exp.cgh[i,]))) == ncol(exp.cgh)) {
            next
        }
	...
	for (i in 1:nrow(exprs(exp.cgh)))
        ...
        if (sum(is.na(assayData(exp.cgh[i,]))) == ncol(exp.cgh)) {
            next
        }
